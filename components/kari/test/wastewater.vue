<script setup>
// 데이터 정의
const wastewaterData = reactive({
  volume: ref(4750), // 용량 (L)
  temperature: ref(38.5), // 온도 (°C)
  salinity: ref(22.7), // 염도 (%)
  pH: ref(8.2), // pH
  nickelContent: ref(356) // Ni 함량 (ppm)
});

// 최대값 및 임계값 정의
const thresholds = {
  volume: { max: 5000, warning: 4500, critical: 4800 },
  temperature: { min: 30, max: 45, optimal: { min: 35, max: 40 } },
  salinity: { min: 5, max: 40, optimal: { min: 15, max: 25 } },
  pH: { min: 5, max: 10, optimal: { min: 7, max: 8.5 } },
  nickelContent: { min: 0, max: 500, warning: 300, critical: 400 }
};
...
const metrics = [
  {
    id: 'volume',
    label: '① 용량',
    value: computed(() => wastewaterData.volume),
  ...
  },
  {
    id: 'temperature',
    label: '② 온도',
    value: computed(() => wastewaterData.temperature),
  ...
  },
  {
    id: 'salinity',
    label: '③ 염도',
    value: computed(() => wastewaterData.salinity),
    format: (val) => Number(val).toFixed(1),
    unit: '%',
  ...
  },
  {
    id: 'pH',
    label: '④ pH',
    value: computed(() => wastewaterData.pH),
    format: (val) => Number(val).toFixed(1),
    unit: '',
  ...
  },
  {
    id: 'nickelContent',
    label: '⑤ Ni 함량',
    value: computed(() => wastewaterData.nickelContent),
    format: (val) => Number(val).toFixed(0),
    unit: 'ppm',
...
onMounted(() => {
  ...
  const interval = setInterval(() => {
    // 약간의 변동성 추가
    wastewaterData.volume = Math.max(3000, Math.min(5000, wastewaterData.volume + (Math.random() * 200 - 100)));
    wastewaterData.temperature = Math.max(30, Math.min(45, wastewaterData.temperature + (Math.random() * 1 - 0.5)));
    wastewaterData.salinity = Math.max(5, Math.min(40, wastewaterData.salinity + (Math.random() * 2 - 1)));
    wastewaterData.pH = Math.max(5, Math.min(10, wastewaterData.pH + (Math.random() * 0.4 - 0.2)));
    wastewaterData.nickelContent = Math.max(0, Math.min(500, wastewaterData.nickelContent + (Math.random() * 20 - 10)));
  }, 5000);
...
<template>
  <div class="w-1/4 bg-gradient-to-br from-indigo-800 to-blue-900 rounded-xl border border-indigo-700 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
    <!-- 카드 헤더 -->
    <div class="relative px-4 py-3 border-b border-indigo-700/40 bg-gradient-to-b from-indigo-800/90 to-indigo-900/80">
      <!-- 배경 효과 -->
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute -top-20 -left-20 w-40 h-40 bg-indigo-500/30 rounded-full blur-3xl opacity-40"></div>
        <div class="absolute top-5 right-10 w-32 h-32 bg-blue-500/30 rounded-full blur-3xl opacity-40"></div>
      </div>
    ...
    <!-- 카드 본문 - 측정값 표시 -->
    <div class="p-4 space-y-3.5 bg-gradient-to-b from-indigo-900/60 to-blue-900/80">
      <!-- v-for를 사용하여 각 측정 항목 렌더링 -->
      <div v-for="metric in metrics" :key="metric.id" class="flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="mr-3 h-6 w-6" :class="metric.iconColor" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="metric.icon" />
        </svg>
        
        <div class="flex-grow">
          <div class="flex justify-between items-center mb-1">
            <div class="text-indigo-100">{{ metric.label }}</div>
            <div class="flex items-baseline">
              <span class="text-sm font-semibold text-white">{{ metric.format(metric.value.value) }}</span>
              <span v-if="metric.unit" class="text-xs ml-1 text-indigo-200">{{ metric.unit }}</span>
            </div>
          </div>
          <div class="relative h-1.5 bg-indigo-900/80 rounded-full overflow-hidden backdrop-blur-sm">
            <div 
    ...
    <!-- 카드 푸터 -->
    <div class="p-4 border-t border-indigo-700/30 flex justify-between items-center text-xs text-indigo-200 bg-gradient-to-b from-blue-900/80 to-indigo-900/80">
