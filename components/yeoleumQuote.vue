...
const services = ref([
	{desc: '웹기획', price: 291414, qty: 1, mm: 1, days: 15},
	{desc: '웹디자인', price: 217843, qty: 1, mm: 1, days: 15},
	{desc: '웹퍼블리싱', price: 217843, qty: 1, mm: 1, days: 15},
	{desc: '웹프론트엔드 개발', price: 311962, qty: 1, mm: 1, days: 25},
	{desc: '웹백엔드 개발 (DB)', price: 311962, qty: 1, mm: 1, days: 25},
])
...
const subtotal = services.value.reduce((acc,cur) => acc + cur.price * cur.days, 0)
...
<template>
	...
	<div class="container">
		<div class="wrap p-2 pb-4 fs-6">
			...
			<div class="row ">
				...
				<div class="col-2 title text-end">Price/day</div>
				<div class="col-2 title text-end">Days</div>
				<div class="col-2 title text-end">Total</div>
			</div>

			<div class="row  pt-1" v-for="(item, i) in services">
				<div class="col-2 pe-3 text-end service-number">#0{{i+1}}</div>
				<div class="col-4">{{item.desc}}</div>
				<div class="col-2 text-end">\{{item.price.toLocaleString()}}</div>
				<div class="col-2 text-end">{{item.days}}</div>
				<div class="col-2 text-end">\{{(item.price * item.days).toLocaleString()}}</div>
			</div>
			<div class="mb-20" />
			...
			<div class="row ">
				<div class="col title text-end">
					...
					<div class="mt-5">VAT.</div>
					<div class="mt-5">Total</div>
				</div>
				<div class="col-2 text-end">
					<div class="">\{{subtotal.toLocaleString()}}</div>
					<div class="mt-5">\{{vat.toLocaleString()}}</div>
					<div class="mt-5"><b>\{{total.toLocaleString()}}</b></div>
				</div>
			</div>
		</div>
	</div>

	<div class="row mx-0" style="background:#0139d6">
		<div class="container">
			<div class="row ps-4 pe-2 py-3 text-white" style="background:#0139d6">
				<div class="col-2"></div>
				<div class="col-2"></div>
				<div class="col-2"></div>
	...
	<div class="container">
		<div class="row" style="font-size:.91rem">
